...
InstallMethod( Category,
  ...
  function( Qq )
    local domain, paths, p, q, parity, quiver, C;
    
    domain := LeftActingDomain( Qq );
    ...
    SetUnderlyingQuiverAlgebra( C, Qq );
...
InstallMethod( Category,
  ...
  function( quiver )
    ...
    return Category( PathAlgebra( ALGEBRIODS.ring, quiver ) );
...
InstallMethod( Category,
  ...
  function( quiver, L )
    ...
    Qq := PathAlgebra( ALGEBRIODS.ring, quiver );
    
    L := List( L, a -> PathAsAlgebraElement( Qq, a[1] ) - PathAsAlgebraElement( Qq, a[2] ) );
    
    return Category( Qq / Ideal( Qq, L ) );
