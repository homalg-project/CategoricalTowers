...
gap> i := InitialObject( L );;
gap> t := TerminalObject( L );;
...
gap> i_op := InitialObject( L_op );;
gap> t_op := TerminalObject( L_op );;
...
gap> alpha := MorphismConstructor( aT, "g_ab", bT ) / L;;
gap> beta := MorphismConstructor( cT, "g_cd", dT ) / L;;
gap> gamma := MorphismConstructor( bT, "g_bc", cT ) / L;;
