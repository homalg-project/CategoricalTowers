...
test-spacing:
	...
	for filename in gap/*; do \
		echo $$filename; \
		echo "LoadPackage(\"ExteriorPowersCategories\"); SizeScreen([4096]); func := ReadAsFunction(\"$$filename\"); FileString(\"gap_spacing\", DisplayString(func));" | gap --quitonbreak --banner; \
		echo -e "\033[0m"; \
