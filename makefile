...
test-spacing:
	...
	for filename in gap/*; do \
		...
		echo "LoadPackage(\"CatReps\"); SizeScreen([4096]); func := ReadAsFunction(\"$$filename\"); FileString(\"gap_spacing\", PrintString(func));" | gap; \
...
ci-test: test-overfull-hboxes test-with-coverage
