...
##
InstallMethod( AffineVarietyOfBimonoids2,
        "for a finitely presented matrix algebra and two records",
        [ IsObjectInCategoryOfFpMatrixAlgebras, IsRecord, IsRecord ],
        
  function( fp_matrix_algebra, counit_record, comult_record )
    local bimonoid, FpMatBialg_R, FpMatAlg_R, B, R, k, AffAlg_k, eta, triple, datum, L, nr_rels, rels,
          ...
          op_triple, obstruction_l, obstruction_r, obstruction2, relations2, bimonoid_variety, map,
          obstruction3, relations3, obstruction4, relations4;
    
    bimonoid := Bimonoid( fp_matrix_algebra, counit_record, comult_record );

    FpMatBialg_R := CapCategory( bimonoid );
    FpMatAlg_R := UnderlyingCategory( FpMatBialg_R );
    ...
    triple := DefiningTripleOfFinitelyPresentedMatrixBialgebra( bimonoid );
    
    datum := DefiningSeptupleOfFinitelyPresentedAlgebra( DefiningPairOfFinitelyPresentedMatrixAlgebra( triple[1] )[1] );
    
    L := datum[1];
    nr_rels := datum[5];
    rels := datum[6];
    
    list_of_comult_matrix_images := ListOfMatrixImages( FpMatAlg_R, triple[3] );
    ...
    target_rep_obj := DefiningPairOfFinitelyPresentedMatrixAlgebra( Target( triple[3] ) )[2][1];
    ...
    bimonoid_variety := ClosedSubsetOfSpec( UnionOfRows( relations3, relations4 ) );
    SetParametrizedObject( bimonoid_variety, bimonoid );
