...
#!     3 := [ 1, 1 ], 4 := [ 1, 1 ] } )
DecoratedFinQuivers :=
  CategoryOfDecoratedQuiversEnrichedOver( decorating_quiver,
          [ "magenta", "cyan" ],
          [ "red", "brown", "orange", "blue", "green" ] );
#! CategoryOfDecoratedQuiversEnrichedOver( decorating_quiver )
B := TerminalObject( DecoratedFinQuivers );
#! <An object in CategoryOfDecoratedQuiversEnrichedOver( decorating_quiver )>
Display( B );
#! ( { 0 := "magenta", 1 := "cyan" },
#!   { 0 := [ 0, 0, "red" ], 1 := [ 1, 0, "brown" ], 2 := [ 0, 1, "orange" ],
#!     3 := [ 1, 1, "blue" ], 4 := [ 1, 1, "green" ] } )
iota := Subobject( B, [ 0 ], [ 3, 4 ] );
#! <A monomorphism in CategoryOfDecoratedQuiversEnrichedOver( decorating_quiver )>
Display( iota );
#! Image of <(V)>:
#! { 0, 1 } ⱶ[ 0, 1 ]→ { 0, 1 }
#! 
...
#! { 0, 1 } ⱶ[ 3, 4 ]→ { 0,..., 4 }
#! 
#! A morphism in CategoryOfDecoratedQuiversEnrichedOver( decorating_quiver )
