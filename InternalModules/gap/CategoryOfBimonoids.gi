...
InstallMethod( LinearizeInternalBimonoid,
  ...
  function( B, bimonoid_in_sFinSets )
    ...
    Assert( 0, HasCommutativeSemiringOfLinearCategory( B ) );
...
InstallMethod( AffineVarietyOfBimonoids,
  ...
  function( monoid, counit_record, comult_record )
    local B, bimonoid, obstruction, R, extract_matrix, relations, bimonoid_variety;
    
    B := UnderlyingCategory( CapCategory( monoid ) );
    
    if not HasCommutativeSemiringOfLinearCategory( B ) then
        TryNextMethod( );
    fi;
    ...
    R := UnderlyingRing( UnderlyingCategory( CapCategory( bimonoid ) ) );
    ...
    relations := List( obstruction, obs -> extract_matrix( obs[1][1] ) - extract_matrix( obs[1][2] ) );
    relations := Filtered( relations, mat -> not IsZero( mat ) );
    relations := List( relations, ConvertMatrixToColumn );
    relations := List( relations, mat -> CertainRows( mat, NonZeroRows( mat ) ) );
    relations := UnionOfRows( R, 1, relations );
