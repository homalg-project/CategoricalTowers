...
InstallMethod( CreatePreSheaf,
  ...
  function ( F )
    ...
    if IsFreeCategory( B_op ) then
        B := OppositeFreeCategory( B_op );
...
InstallGlobalFunction( OppositeOfFpEnrichedCategory,
  function ( B )
    ...
    if IsFreeCategory( B ) then
        return OppositeFreeCategory( B : FinalizeCategory := true );
        ...
        Error( "the first argument must be in { IsFreeCategory, IsCategoryFromNerveData, IsCategoryFromDataTables, IsFinite, IsAlgebroid }\n" );
    fi;
...
InstallGlobalFunction( ADD_FUNCTIONS_FOR_WELL_DEFINED_TO_PRESHEAF_CATEGORY,
  function ( PSh )
    ...
    if IsFreeCategory( B ) then
        ...
        relations := RelationsOfFreeCategory( B_op );
        A := kq;
...
InstallMethodWithCache( PreSheavesOfFpEnrichedCategory,
  ...
  function ( CAP_NAMED_ARGUMENTS, B, D )
    ...
    ##
    if ( IsFreeCategory( B ) and HasIsFinitelyPresentedCategory( B ) and IsFinitelyPresentedCategory( B ) ) or
    ...
    if CAP_NAMED_ARGUMENTS.no_precompiled_code <> true then
        ...
        if IsFreeCategory( B ) and IsSkeletalCategoryOfFiniteSets( D ) then
            
          ADD_FUNCTIONS_FOR_PreSheavesOfFreeCategoryInSkeletalFinSetsPrecompiled( PSh );
            
            ADD_FUNCTIONS_FOR_PreSheavesOfFreeCategoryInSkeletalFinSetsSubobjectClassifierPrecompiled( PSh );
