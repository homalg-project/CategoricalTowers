...
test-notebooks:
	...
	for filename in *.ipynb; do \
		...
		cat "$$filename" | perl -0777 -pe 's/\n(\s+)\]\n\s+\},?\n\s+{\n\s+"name": "std(?:out|err)",\n\s+"output_type": "stream",\n\s+"text": \[/,/g' | grep -v '"version": "' | grep -v 'CapAndHomalg v' | grep -v 'Generated by graphviz version ' | grep -v '^\s*"\\r"' | perl -0777 -pe 's/,\n(\s*\])/\n$$1/g' | perl -0777 -pe 's/\n[ \t]*\{[^{}]*"output_type":\s*"stream"[^{}]*"text":\s*\[\s*\][^{}]*\},?//g' > modified_in; \
		cat out | perl -0777 -pe 's/\n(\s+)\]\n\s+\},?\n\s+{\n\s+"name": "std(?:out|err)",\n\s+"output_type": "stream",\n\s+"text": \[/,/g' | grep -v '"version": "' | grep -v 'CapAndHomalg v' | grep -v 'Generated by graphviz version ' | grep -v '^\s*"\\r"' | perl -0777 -pe 's/,\n(\s*\])/\n$$1/g' | perl -0777 -pe 's/\n[ \t]*\{[^{}]*"output_type":\s*"stream"[^{}]*"text":\s*\[\s*\][^{}]*\},?//g' > modified_out; \
