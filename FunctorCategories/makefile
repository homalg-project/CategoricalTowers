...
test-notebooks:
	...
	for filename in *.ipynb; do \
		...
		cat "$$filename" | grep -v '"version": "' | grep -v 'CapAndHomalg v' | grep -v 'Generated by graphviz version ' | perl -0777 -pe 's/,\n(\s*\])/\n$$1/g' | perl -0777 -pe 's/\n[ \t]*\{[^{}]*"output_type":\s*"stream"[^{}]*"text":\s*\[\s*\][^{}]*\},?//g' > modified_in; \
		cat out | grep -v '"version": "' | grep -v 'CapAndHomalg v' | grep -v 'Generated by graphviz version ' | perl -0777 -pe 's/,\n(\s*\])/\n$$1/g' | perl -0777 -pe 's/\n[ \t]*\{[^{}]*"output_type":\s*"stream"[^{}]*"text":\s*\[\s*\][^{}]*\},?//g' > modified_out; \
...
test-spacing:
	...
	for filename in gap/*; do \
		echo $$filename; \
		gap --quitonbreak --norepl --banner -c "LoadPackage(\"FunctorCategories\"); SizeScreen([4096]); func := ReadAsFunction(\"$$filename\"); FileString(\"gap_spacing\", DisplayString(func));"; \
